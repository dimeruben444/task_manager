...
class Task(models.Model):
    ...
    # Campos principales (Obligatorios en el alta rápida)
    ...
    urgency = models.PositiveSmallIntegerField(
        _('Urgencia (1-4)'),
        validators=[ #valida que este entre 1 y 4
    ...
    # Campos opcionales (Ocultos en desplegable 'Opciones Avanzadas')
    ...
    def calculate_quadrant(self): #Calculo del cuadrante basándo en la puntuación de 1 a 4 estrellas.        
    ...
    def check_auto_urgency(self):
        if self.due_date and not self.is_completed: #si hay fecha de expiracion y la tarea no esta completada 
            time_left = self.due_date - timezone.now() 
            if time_left <= timedelta(hours=48) and self.urgency <= 2:# Si quedan menos de 48h setea la urgencia al máximo
                self.urgency = 4  


    def save(self, *args, **kwargs):
        # Comprobar vencimiento cercano
        self.check_auto_urgency()


        if not self.board and self.assigned_to:
            default_board = Board.objects.filter(owner=self.assigned_to, is_default=True).first()
            if default_board:
        ...
        super().save(*args, **kwargs) #guardar
